...
tmp <- comp_asdrs_boot %>%
  ...
  ungroup() %>%
  group_by(method, sex, agegp10) %>%
  summarize(bias_mean = mean(bias),
            bias_ci_low = quantile(bias, .025),
            bias_ci_high = quantile(bias, .975),
            var_mean = mean(var),
            var_ci_low = quantile(var, .025),
            var_ci_high = quantile(var, .975),
            mse_mean = mean(mse),
            mse_ci_low = quantile(mse, .025),
            mse_ci_high = quantile(mse, .975))
```
...
tmp <- comp_asdrs_boot %>%
  ...
  pivot_wider(names_from = 'method',
              values_from = c('bias', 'var', 'mse', 'rmse'))


#enter <- colMeans(tmp[,c('var_Network', 'var_Sibling')])
#cov_matrix <- cov(tmp[,c('var_Network', 'var_Sibling')])

#ggplot(tmp) +
#  geom_point(aes(x=var_Network,
#                 y=var_Sibling)) +
#  geom_ellipse(aes(x0 = center['var_Network'],
#                   y0 = center['var_Sibling'],
#                   a = sqrt(cov_matrix[1,1]) * 1.96,  # semi-major axis
#                   b = sqrt(cov_matrix[2,2]) * 1.96,  # semi-minor axis
#                   angle = atan2(2 * cov_matrix[1,2], cov_matrix[1,1] - cov_matrix[2,2]) / 2),
#    color = "blue",
#    fill = NA,
#    linewidth = 1
#  )  
...
tmp %>%
  ...
  geom_point(aes(x=rmse_Network,
                 ...
                 color=agegp10),
             alpha=.02) +
  geom_point(data=tmp_agg,
             aes(x=mean_rmse_Network,
                 y=mean_rmse_Sibling,
                 color=agegp10)) +
  #stat_ellipse(aes(x=rmse_Network,
  geom_density_2d(aes(x=rmse_Network,
                      y=rmse_Sibling,
                      group=agegp10,
                      color=agegp10),
                  contour=TRUE) + 
...
tmp %>%
  ...
  geom_point(aes(x=sqrt(var_Network),
             ...
             alpha=.05) +
  geom_point(data=tmp_agg,
             #aes(x=sqrt(mean_var_Network),
             #    y=sqrt(mean_var_Sibling),
  ...
  xlim(0,50) + ylim(0,50) +
  facet_wrap(~sex) +
  coord_obs_pred(xlim=c(0,10), ylim=c(0,10)) 
```
