...
```{r}
errs_to_save <- bind_rows(
        asdr_tot_properties %>%
          rename_with(~ str_replace(.x, 'asdr_', ''), matches('asdr_')), 
        prob_tot_properties %>%
          rename_with(~ str_replace(.x, 'prob_', ''), matches('prob_')), 
        )
```
...
err_diffs_jab <- bind_rows(
  ...
  select(qty, loss, geo, everything())
...
##   agesex_err_diffs_summ ===> asdr_agesex_properties
##   geo_agesex_err_diffs_summ ====> asdr_properties
OLD_err_breakdown_plot <- function(err_metric, 
                               err_metric_name, 
                               diff = FALSE) {
