...
fig_diff_asdr <-
 comp_asdrs_boot %>%
   ...
   facet_grid(~ label) +
   xlab("") +
   ylab("Average Estimated Difference\nin Death Rate (per 1,000)") +
   #ylab("") +
   theme_minimal() +
   xaxistext
...
loss_plot <- function(cur_qoi_var, 
  ...
  fig_diff <- diff_data %>%
    ...
    # Red annotations
    annotate(geom = 'text', 
             label = "Network is worse ->", 
             y = annheight, x = 0.25, 
             size = annrelsize, color = 'red', angle = 90) +
    annotate(geom = 'text', 
             label = "<- Sibling is worse", 
             y = -annheight, x = 0.25, 
             size = annrelsize, color = 'red', angle = 90) +
    coord_cartesian(clip = "off", xlim = c(0.5, 5.5)) +
