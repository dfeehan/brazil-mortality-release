...
#ci_coverage_plot <- ci_coverage_df %>%
#  ggplot(.) +
#  geom_point(aes(x=method, y=ci_covered)) +
...
#
#ggsave(filename=file.path(comp_out_dir, "ci_coverage_plot.pdf"),
...
jab_agesex_asdr_ests <- jab_asdr_ests %>%
  ...
  mutate(across(
    ...
    .names = "{str_replace(.col, 'var_jab', 'se_jab')}")) %>%
  mutate(qty = 'asdr') %>%
  ## TEMP NEW - conservative variance ests for systematic bias
