...
This is a helper function that will be useful below
...
Get the city-specific estimated differences in RMSE

```{r}
# uncertainty in estimated point estimates (based on JAB)
lopo_rmse_city_diff <- lopo_agesexgeo_wide %>%
  ...
  mutate(city_se_rmse_diff = sqrt(city_var_rmse_asdr_lopo_n_minus_s)) 

# point estimate (not based on JAB)
rmse_city_diff <- estimated_properties %>%
  mutate(rmse_asdr_n_minus_s = rmse_Network - rmse_Sibling) %>%
  group_by(geo) %>%
  summarize(city_rmse_diff = mean(rmse_asdr_n_minus_s)) %>%
  # join on the uncertainty in estimated RMSE diffs, based on JAB
  left_join(lopo_rmse_city_diff) %>%
  mutate(city_rmse_diff_ci_low = city_rmse_diff - 1.96*city_se_rmse_diff,
         city_rmse_diff_ci_high = city_rmse_diff + 1.96*city_se_rmse_diff)

```

```{r}
biggestdiff <- max(c(1000*rmse_city_diff$city_rmse_diff_ci_low,
                     1000*rmse_city_diff$city_rmse_diff_ci_high))
annscale <- .8
annxoff <- 0.2 

rmse_city_diff %>%
  mutate(geo = fct_reorder(geo, city_rmse_diff)) %>%
  ggplot(.) +
  geom_pointrange(aes(x = geo,
                      y = 1000*city_rmse_diff,
                      ymin = 1000*city_rmse_diff_ci_low,
                      ymax = 1000*city_rmse_diff_ci_high)) +
