...
all_metrics_re <- "^absbias_|^se_|^mare_|^mse_|^rmse_|^bias_|^var_"
...
asdr_tot_properties_tosave <-
  ...
  mutate(
    ...
    loss = str_extract(full_name, "(se|rmse|mare|mse|absbias|bias|var|mean)(?=_asdr|_Network|_Sibling|_diff|$)")
  ) %>%
...
loss_plot <- function(cur_qoi_var, 
  ...
  annrelsize <- 3 
  annheight <- max_abs_diff * 0.5 # Dynamic height based on data range
...
bias_plot <- function(cur_qoi_var, 
  ...
  # right panel: absolute value of bias 
  diff_data <- df %>%
    #filter(method == 'diff') %>%
    filter(method != 'diff') %>%
...
fig_diff <- diff_data %>%
  mutate(method = dplyr::recode(method, !!!method_facet_names)) %>%
  ggplot(aes(x = agegp10, y = cur_qoi, ymin = cur_ci_low, ymax = cur_ci_high, 
             color = sex, shape = method)) +
  geom_hline(yintercept = 0, color = 'grey') +
  geom_pointrange(position = position_dodge(width = 0.4)) +
  scale_y_continuous(limits = c(0, max_abs_diff)) +
  #annotate(geom = 'text', 
  #         label = "Sibling is better ->", 
  #         y = annheight, x = 0.25, 
  #         size = annrelsize, color = 'black', angle = 90) +
  ...
  #         size = annrelsize, color = 'black', angle = 90) +
  #coord_cartesian(clip = "off", xlim = c(0.5, 5.5)) +
  xlab("") + ylab("| Average Estimated Bias |") +
  theme_minimal()
...
fig_combinedbias <- bias_plot(point_est_bias, 
                              ...
                              diff_var = point_est_absbias,
                              diff_se = se_jab_absbias_asdr_lopo)
...
```{r}
fig_combinedbias$plot_df %>%
  group_by(method) %>%
  summarize(mean_abs_bias = mean(point_est_absbias))
```
