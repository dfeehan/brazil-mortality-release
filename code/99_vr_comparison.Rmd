...
And also calculate these quantities for the difference between network and sibling

```{r}
asdr_diff_properties_point_estimates <- asdr_properties_point_estimates %>%
  pivot_wider(names_from = 'method',
              values_from = c('est_rmse', 'est_mse', 'est_bias', 'est_var', 'est_mare')) %>%
  mutate(est_rmse_n_minus_s = est_rmse_Network - est_rmse_Sibling,
         est_mare_n_minus_s = est_mare_Network - est_mare_Sibling)

prob_diff_properties_point_estimates <- prob_properties_point_estimates %>%
  pivot_wider(names_from = 'method',
              values_from = c('est_rmse', 'est_mse', 'est_bias', 'est_var', 'est_mare')) %>%
  mutate(est_rmse_n_minus_s = est_rmse_Network - est_rmse_Sibling,
         est_mare_n_minus_s = est_mare_Network - est_mare_Sibling)
...
jab_ests <- lopo_ests %>%
  ...
  summarize(# number of PSUs used in JAB calculation
            ...
            # JAB variance estimate of MARE estimate
            var_mare_jab = jack.var(mare_asdr_lopo),
...
jab_prob_ests <- lopo_prob_ests %>%
  ...
  summarize(# number of PSUs used in JAB calculation
            ...
            # JAB variance estimate of MARE estimate
            var_mare_jab = jack.var(mare_prob_lopo),            
            # JAB variance estimate of bias estimate
...
lopo_ests_wide <- lopo_ests %>%
  ...
  mutate(rmse_asdr_lopo_n_minus_s = rmse_asdr_lopo_Network - rmse_asdr_lopo_Sibling,
         mare_asdr_lopo_n_minus_s = mare_asdr_lopo_Network - mare_asdr_lopo_Sibling) 
...
#### Get JAB uncertainty estimates in ASDR error differences by age
...
jab_age_diff_ests <- jab_diff_ests %>% 
...
asdr_diff_properties <- asdr_diff_properties_point_estimates %>%
  ...
  left_join(jab_diff_ests %>% select(-starts_with('var')))
...
TODO - LEFT OFF HERE
- save for plots
- run for vr / adj vr and for all geos
