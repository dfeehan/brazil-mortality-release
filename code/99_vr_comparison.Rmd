...
prob_properties_point_estimates <- comp_probs_boot %>%
  ...
  summarize( est_bias = mean(post_mean_q) - gs_prob[1],
             est_absbias = mean(abs(post_mean_q - gs_prob[1])),
                     ...
                     function(jab_boot_indexes, cur_geo) {
                        cur_res <- comp_asdrs_boot %>%
                          ...
                          summarize(n_jab_check = n(),
                                    ...
                                    absbias_asdr_lopo = mean(abs(post_mean_asdr - gs_asdr)),
                     ...
                     function(jab_boot_indexes, cur_geo) {
                        cur_res <- comp_probs_boot %>%
                          ...
                          summarize(n_jab_check = n(),
                                    ...
                                    absbias_prob_lopo = mean(abs(post_mean_q - gs_prob)),                                    
...
loss_plot <- function(cur_qoi_var, 
  ...
  # if is_bias, need the difference also
  if (is_bias) {
  ...
  # if not bias, use the 'diff' metric
  if (! is_bias) {
    diff_data <- plot_df %>% filter(method %in% c('diff'))
  } else {
    # otherwise, if is_bias, then use different metric 
    diff_data <- df %>%
      filter(method == 'diff') %>%
...
fig_combinedbias <- loss_plot(point_est_bias, 
