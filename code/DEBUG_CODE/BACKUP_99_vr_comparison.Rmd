...
asdr_properties_point_estimates <- comp_asdrs_boot %>%
  ...
  summarize( asdr_bias = mean(post_mean_asdr) - gs_asdr[1],
             asdr_mse = mean( (post_mean_asdr - gs_asdr[1])^2 ),
             asdr_mare = mean( abs(post_mean_asdr - gs_asdr[1])/gs_asdr[1]),
             asdr_var = fp.var(post_mean_asdr)
  ...
  summarize(# number of PSUs used in JAB calculation
            ...
            # JAB variance estimate of RMSE estimate
            var_rmse_jab = jack.var(rmse_asdr_lopo),
            # JAB variance estimate of bias estimate
            var_bias_jab = jack.var(bias_asdr_lopo)) %>%
  mutate(se_var_jab = sqrt(var_var_jab),
         se_mse_jab = sqrt(var_mse_jab),
         se_rmse_jab = sqrt(var_rmse_jab),
         se_bias_jab = sqrt(var_bias_jab))
```
...
## Calculate the leave-one-PSU-out (LOPO) estimates for death rates

This takes about 6 minutes on my fancy laptop...
  ...
  mutate(
    ...
    lopo_ests = map2(jab_indexes, 
                     ...
                     function(jab_boot_indexes, cur_geo) {
                        ...
                        # repeat for probabilities
                        cur_res_probs <- comp_probs_boot %>%
                          filter(geo == cur_geo) %>%
                          # filter to boot indexes to be used for the hold-one-out calculations
                          # based on this index
                          filter(boot_idx %in% jab_boot_indexes) %>%
                          ...
                          summarize(n_jab_check = n(),
                                    ...
                                    .groups = "drop") %>%
                          mutate(rmse_asdr_lopo = sqrt(mse_prob_lopo))
...
## Calculate Var/MSE/Bias estimates using the LOPO estimates
...
```{r jab-prob-estimates}
jab_prob_ests <- lopo_prob_ests %>%
  select(-jab_indexes) %>%
  # geo is stored in lopo_ests, and we don't want to duplicate it
  select(-geo) %>%
  unnest(cols = lopo_ests) %>%
  ...
  summarize(# number of PSUs used in JAB calculation
            ...
            n_jab_avg = mean(n_jab), 
            # JAB variance estimate of variance estimate
            var_var_jab = jack.var(var_prob_lopo),
            # JAB variance estimate of MSE estimate
            var_mse_jab = jack.var(mse_prob_lopo),
            # JAB variance estimate of RMSE estimate
            var_rmse_jab = jack.var(rmse_prob_lopo),
            # JAB variance estimate of bias estimate
            var_bias_jab = jack.var(bias_prob_lopo)) %>%
  mutate(se_var_jab = sqrt(var_var_jab),
         se_mse_jab = sqrt(var_mse_jab),
         se_rmse_jab = sqrt(var_rmse_jab),
         se_bias_jab = sqrt(var_bias_jab))
...
jab_agesexgeo_rmse_diff <- lopo_agesexgeo_wide %>%
  ...
  summarize(# aggregate across the LOPO values with the jackknife variance formula
            geo_var_rmse_asdr_lopo_n_minus_s_jab = jack.var(mean_rmse_asdr_lopo_n_minus_s),
            # and this is the mean difference across LOPO values
            geo_mean_rmse_asdr_lopo_n_minus_s_jab = mean(mean_rmse_asdr_lopo_n_minus_s),
            .groups='drop') %>%
  mutate(geo_se_rmse_diff_jab = sqrt(geo_var_rmse_asdr_lopo_n_minus_s_jab)) 
```

Repeat for probabilities

```{r}
lopo_agesexgeo_prob_wide <- lopo_prob_ests %>%
  # geo is stored in lopo_ests, and we don't want to duplicate it
  select(-geo) %>%
  unnest(lopo_ests) %>%
  pivot_wider(names_from = 'method',
              values_from = 'rmse_prob_lopo',
              id_cols=c('cb_code', 'geo', 'sex')) %>%
  rename(rmse_prob_lopo_net = Network,
         rmse_prob_lopo_sib = Sibling) %>%
  mutate(rmse_prob_lopo_n_minus_s = rmse_prob_lopo_net - rmse_prob_lopo_sib) 

# uncertainty in estimated point estimates of probabilities (based on JAB)
jab_agesexgeo_prob_diff <- lopo_agesexgeo_prob_wide %>%
  ## now use the jackknife formula to get variance estimates for each city
  group_split(geo) %>%
  map_dfr(~ {
    .x %>%
      # aggregate LOPO estimates across sexes within this city and PSU
      group_by(geo, cb_code) %>%
      summarize(mean_rmse_prob_lopo_n_minus_s = mean(rmse_prob_lopo_n_minus_s),
                .groups='drop')
                # this should be 10 (since there are 10 age/sex groups)
                #num_gp_check = n())
  }) %>%
  ...
  group_by(geo) %>%
  # estimated variance in city MSE difference estimate, using JAB
  summarize(geo_var_rmse_prob_lopo_n_minus_s_jab = jack.var(mean_rmse_prob_lopo_n_minus_s),
            geo_mean_rmse_prob_lopo_n_minus_s_jab = mean(mean_rmse_prob_lopo_n_minus_s),
            .groups='drop') %>%
  mutate(geo_se_rmse_prob_diff_jab = sqrt(geo_var_rmse_prob_lopo_n_minus_s_jab)) 

```


## Calculate summary measures of error
...
## Investigate the Var/MSE/Bias estimates in various ways
...
rmse_geo_diff <- point_estimates_properties %>%
  ## TODO ADAPT THIS - point_estimates_properties is long, need it to be wide...
  #%>%
#  pivot_wider(names_from = 'method',
#              values_from = c('bias', 'var', 'fp.var', 'mse', 'rmse',
#                              starts_with('var'))) %>%
#  mutate(qty = 'asdr') %>%
  mutate(rmse_asdr_n_minus_s = rmse_Network - rmse_Sibling) %>%
  group_by(geo) %>%
  summarize(geo_rmse_diff = mean(rmse_asdr_n_minus_s)) %>%
  # join on the uncertainty in estimated RMSE diffs, based on JAB
  left_join(lopo_rmse_geo_diff) %>%
  mutate(geo_rmse_diff_ci_low = geo_rmse_diff - 1.96*geo_se_rmse_diff,
         geo_rmse_diff_ci_high = geo_rmse_diff + 1.96*geo_se_rmse_diff)

```

```{r}
biggestdiff <- max(c(1000*rmse_geo_diff$geo_rmse_diff_ci_low,
                     1000*rmse_geo_diff$geo_rmse_diff_ci_high))
...
rmse_geo_diff %>%
  mutate(geo = fct_reorder(geo, geo_rmse_diff)) %>%
  ggplot(.) +
  geom_pointrange(aes(x = geo,
                      y = 1000*geo_rmse_diff,
                      ymin = 1000*geo_rmse_diff_ci_low,
                      ymax = 1000*geo_rmse_diff_ci_high)) +
...
rmse_tot_diff <- rmse_geo_diff %>%
  ...
  summarize(# overall estimate mse diff is the average across cities
            ...
            est_var_rmse_diff = ((1/NUM_GEO_UNITS)^2)*sum(geo_var_rmse_asdr_lopo_n_minus_s)) %>%
...
biggestdiff <- max(c(1000*rmse_tot_diff$est_ci_low_rmse_diff, 
                     1000*rmse_tot_diff$est_ci_high_rmse_diff))
...
### Headline variance, bias, MSE

TODO


### Age/sex breakdown of variance, bias, MSE 
