...
scale_asdrs <- function(x) {
  ...
  x %>%
    mutate(across(any_of(cols_to_scale), ~ .x ))
}
```
...
TODO - LEFT OFF HERE: adapting this and below to se
(should also do abs bias)
...
```{r}
NUM_GEO_UNITS_NOTO <- 26

  #summarize(across(starts_with('var'), 
  #                 ~ ((1/NUM_GEO_UNITS)^2)*sum(.x) ,
  #                 .names = "{.col}")) %>%
jab_agesex_asdr_ests_noTO <- jab_asdr_ests %>%
  filter(geo != 'TO') %>%
  group_by(agegp10, sex) %>%
  # sum variances across geo to get aggregate variance estimate
  # (and note we need to multiply by 1/C^2)
...
```{r}
asdr_agesex_properties_noTO <- asdr_properties_point_estimates %>%
  filter(geo != 'TO') %>%
  rename_with(~ paste0('point_', .x),
              starts_with('est_')) %>%
  # aggregate up to age/sex level (by averaging across cities)
  ...
  pivot_wider(names_from = metric,
              values_from = c(Network, Sibling, diff),
              names_glue = "{metric}_{.value}") %>%
  left_join(jab_agesex_asdr_ests_noTO) 
  # TODO - ADD SE?
```


```{r}
asdr_agesex_properties_long_noTO <- asdr_agesex_properties_noTO %>%
  pivot_longer(
    cols = contains(c("_Network", "_Sibling", "_diff")),
    names_to = c(".value", "method"),
    names_pattern = "(.*)_(Network|Sibling|diff)$"
...
```{r}
fig_var_noTO <- loss_plot(point_est_var, se_jab_var_asdr_lopo, 
                     'Estimated variance (omitting TO)',
                     df=asdr_agesex_properties_long_noTO)
fig_var_noTO
```
...
to_ests %>%
  ...
  ggplot(.,
         aes(x = lopo_ests_mean_asdr_lopo)) +
...
plot for all 27 cities, scales fixed + free
...
tmpplotdf %>%
  ...
  geom_text_repel(aes(x = lopo_ests_mean_asdr_lopo,
                      y = lopo_ests_var_asdr_lopo,
                      label = cb_code),
                  data = . %>% slice_max(lopo_ests_mean_asdr_lopo, n=2) 
                  ) +
  facet_grid(lopo_ests_agegp10 ~ lopo_ests_sex) +
  xlab("LOPO Estimate for ASDR") +
...
# these are the TO outliers for old men
# '0e1df638'
# 'a464aeb3'
```
...
#  xlim(-1,100) +
  ...
  theme_minimal()
```
Look at tails of just TO estimates
...
tmp <-  comp_asdrs_boot %>%
  ...
  filter(method=='Network') %>%
  group_by(geo) %>%
  group_walk(function(dfgeo, gp, ...) {
    print(glue::glue("{gp}"))
    print(summary(dfgeo$post_mean_asdr))
  })
```
...
which specific bootstrap rep has TO estimate at > 600?
=> boot rep idx 2333
...
- I think the idea is that the actual variation in ASDR estimates (from bootstrap, above) has some outliers
- still don't quite understand why this translates to big uncertainty only in this case
- note that there are a very small number of very big estimates in bootstrap distn:

> tmp2[order(-tmp2)]
   [1] 657.93396 380.90553 226.28366 164.54684 161.82671 159.81479 143.49099 137.90082 128.77061 119.29531 107.35993 106.69539 106.34063 100.85421  98.20156  97.58916
  
TODO NEXT
- maybe look at bootstrap resamples w/ three highest estimates and see if there's a single PSU responsible; then, JAB estimates based on that PSU might be wonky?  
- also look at plot above that compares LOPO ASDR ests and LOPO VAR(ASDR) ests; what is the deal with the weird outlier?

TODO - go and look at variation in 'how many deaths' for TO...
...
tmp <-  comp_asdrs_boot %>%
  ...
  filter(method == 'Network') %>%
  group_by(geo) %>%
  summarize(var = var(post_mean_asdr))
...
loss_plot <- function(cur_qoi_var, 
                      ...
                      # if is_bias, then we'll look at the difference in the 
                      # absolute value of the biases, not the difference in the biases;
                      # we'll also put a horizontal line at y=0
                      is_bias = FALSE) {
  
  qoi_name <- as_label(enquo(cur_qoi_var))
  se_name  <- as_label(enquo(cur_se_var))
  ...
  if(is_bias) {
    ...
    diffqoi_name <- str_replace(qoi_name, 'bias', 'absbias')
    diffse_name <- str_replace(se_name, 'bias', 'absbias')
    
    plot_df <- plot_df %>%
      mutate(diff_qoi = .data[[diffqoi_name]],
             diff_se = .data[[diffse_name]])
  }
  ...
  if(! is_bias) {
    ...
    # this is bias
    # ... so, we want a horizontal line at 0 in the method-specific panels
    # and we want to compare the difference in the absolute values of the bias
    ...
    fig_diff <-
     plot_df %>%
       filter(method %in% c('diff')) %>%
       mutate(method = dplyr::recode(method, !!!method_facet_names_bias)) %>%
       ggplot(.) +
       geom_hline(yintercept = 0, color = 'grey') +
       geom_pointrange(aes(x = agegp10,
                           #y = cur_qoi,
                           y = diff_qoi,
                           ymax = diff_qoi + (1.96*diff_se),
                           ymin = diff_qoi - (1.96*diff_se),
                           color = sex
                           ),
                           position = position_dodge(width=0.4)
                       ) +
...
```{r}
fig_se <- loss_plot(point_est_se, se_jab_se_asdr_lopo, 'Estimated se')
fig_se
```

```{r}
fig_absbias <- loss_plot(point_est_absbias, se_jab_absbias_asdr_lopo, 'Estimated absolute bias')
fig_absbias
```
