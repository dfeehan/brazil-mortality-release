...
```{r}
scale_1000x <- function(x) { 1000*x }

err_diffs_summ <- bind_rows(errs_cv,
                           errs_rv,
                           errs_nv,
                           errs_ca,
                           errs_ra,
                           errs_na) %>%
  select(-var_jab) %>%
  filter(loss %in% c('mare', 'rmse')) %>%
  filter(metric == 'diff') %>%
  # make RMSE values x 1,000 (like death rates typically are)
  #mutate(point_est = ifelse(qty == 'asdr' & loss == 'rmse', scale_1000x(point_est), point_est)) %>%
  #mutate(se_jab = ifelse(qty == 'asdr' & loss == 'rmse', scale_1000x(se_jab), se_jab)) %>%
  mutate(ci_low = point_est - (se_jab * 1.96),
         ci_high = point_est + (se_jab * 1.96))

err_diffs_summ
```

Plot w/ pooling for RMSE / ASDR
...
fec.size <- 5 
ggsave(filename=file.path(out_dir, "pooling_rmse.pdf"),
       plot=pr_asdr_rmse,
       height=fec.size,
       width=fec.size)
```
