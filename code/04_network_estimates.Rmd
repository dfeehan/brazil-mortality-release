...
write_csv(n.net,
          file=file.path(out_dir, 'num_interviews.csv'))
write_csv(dpi.net,
          file=file.path(out_dir, 'num_deaths_unweighted_network.csv'))
```
...
# progressr progress bar appearance; see
...
with_progress({
  ...
  netsurv.asdr <- future_map_dfr(all.states,
    function(state) {
      ...
      # update progress bar
      p(message = sprintf("Processing %s", state))  
...
glue::glue("
           In {zerokp} cases out of {total} ({round(100*(zerokp/total),2)} percent), 
           bootstrapped estimate of degree for a given age group is 0, leading 
           to undefined estimated death rates.\n\n
           This table shows whether or not this affects any age groups used in
           the analysis below:\n
           ", 
...
```{r}
log_info("Network estimates finished")
```
