...
sib.asdr <- furrr::future_imap_dfr(city.sib.dat,
                                      function(cur.sib.dat, state) {
                                       ...
                                       #tmp_bw <- readRDS(file.path(survey_data_dir, boot.weights.fn)) 
                                       #
                                       ### reload these weights to avoid memory errors in future... 
                                       # city.tmp_bw <- setNames(map(all.states,
                                       #                                   function(state) {
                                       #                                     cbw <- boot.weights %>% 
                                       #                                       filter(state_abbrev==state) %>% 
                                       #                                       select(-state_abbrev) %>%
                                       ...
                                       #                               all.states)
                                       #                                
                                       # cur.bw <- city.tmp_bw[[state]]
                                        
                                        #tic("Time for one state (RO)")
                                        cur.ests <- 
                                          sibling_estimator(sib.dat = cur.sib.dat,
                                          ego.id = 'ego.id',           # column with the respondent id
                                          sib.id = 'sib.id',           # column with sibling id 
                                          ...
                                          #return.boot=FALSE)          # not returning full bootstrap results for now
                                          return.boot=TRUE)           
